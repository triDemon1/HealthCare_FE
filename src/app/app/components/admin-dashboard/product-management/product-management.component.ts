...
import { Observable, catchError, tap, of, switchMap, Subscription, combineLatest, BehaviorSubject, startWith } from 'rxjs';
...
export class ProductManagementComponent {
  ...
  getPreviousPageIndex(currentPageIndex: number | null): number {
    const safePageIndex = currentPageIndex ?? 0;
    return safePageIndex - 1;
  }

  // Phương thức tính chỉ mục trang kế tiếp
  getNextPageIndex(currentPageIndex: number | null): number {
    const safePageIndex = currentPageIndex ?? 0;
    return safePageIndex + 1;
  }
  ...
  filterByCategory(event: Event, categoryId: number): void {
    ...
    this.pageIndex = 0;
    this.router.navigate(['/admin/products-category', categoryId]);
  }
  ...
  createProduct(): void {
    this.router.navigate(['/admin/products/new']); // Navigate to /admin/products/new
  }
}
